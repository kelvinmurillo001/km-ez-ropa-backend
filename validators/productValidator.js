...
/* -------------------------------------------------------------------------- */
/* 📸 Reglas comunes para validación de imágenes (principal y variantes)      */
/* -------------------------------------------------------------------------- */
...
const variantesRules = [
  ...
  body('variants.*.talla')
    .optional().notEmpty().withMessage('⚠️ Cada variante necesita una talla.'),
  body('variants.*.color')
    .optional().notEmpty().withMessage('⚠️ Cada variante necesita un color.'),
  body('variants.*.imageUrl')
    .optional().notEmpty().withMessage('⚠️ La variante debe incluir una imagen.').bail()
    .isURL().withMessage('⚠️ La URL de la imagen de variante es inválida.'),
  body('variants.*.cloudinaryId')
    .optional().notEmpty().withMessage('⚠️ cloudinaryId requerido para la variante.'),
  body('variants.*.stock')
    .optional()
...
export const createProductValidation = [
  ...
  body('sizes')
    .optional().isArray().withMessage('⚠️ El campo sizes debe ser un arreglo.'),
  body('stock')
    .optional().isInt({ min: 0 }).withMessage('⚠️ El stock debe ser un entero ≥ 0.'),
  body('featured')
    .optional().isBoolean().withMessage('⚠️ featured debe ser booleano.'),
  body('color')
    .optional().isString().withMessage('⚠️ Color inválido.'),
...
export const updateProductValidation = [
  body('name')
    .optional().isString().withMessage('⚠️ El nombre debe ser texto.').bail()
    .isLength({ min: 2 }).withMessage('⚠️ Nombre demasiado corto.'),
  body('price')
    .optional().isFloat({ gt: 0 }).withMessage('⚠️ El precio debe ser positivo.'),
  body('category')
    .optional().isString().withMessage('⚠️ Categoría inválida.'),
  body('subcategory')
    .optional().isString().withMessage('⚠️ Subcategoría inválida.'),
  body('tallaTipo')
    .optional().isString().withMessage('⚠️ Tipo de talla inválido.'),
  body('color')
    .optional().isString().withMessage('⚠️ Color inválido.'),
  body('sizes')
    .optional().isArray().withMessage('⚠️ Sizes debe ser un arreglo.'),
  body('stock')
    .optional().isInt({ min: 0 }).withMessage('⚠️ El stock debe ser un entero ≥ 0.'),
  body('featured')
    .optional().isBoolean().withMessage('⚠️ featured debe ser booleano.'),
