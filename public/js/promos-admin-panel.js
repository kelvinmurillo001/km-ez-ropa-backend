...
document.addEventListener("DOMContentLoaded", async () => {
  ...
  cargarTodasPromos();
  document.getElementById("buscarPromo")?.addEventListener("input", cargarTodasPromos);
  document.getElementById("filtroEstado")?.addEventListener("change", cargarTodasPromos);
});
...
async function cargarTodasPromos() {
  ...
  try {
    const nombre = document.getElementById("buscarPromo")?.value.trim().toLowerCase() || "";
...
function renderPromos(promos = []) {
  ...
  promos
    ...
    .forEach(promo => container.appendChild(crearCardPromo(promo)));
}

function crearCardPromo(promo) {
