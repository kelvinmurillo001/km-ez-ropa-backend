...
function renderizarProducto(p) {
  ...
  const stockHTML = tieneVariantes
    ? `
...
/* 🎨 Variantes: selectores color / talla */
function configurarSelectores(p) {
  const variantes = (p.variants || []).filter(v => v.stock > 0 && v.active !== false);
  ...
  colorSelect.onchange = () => {
    ...
    const tallas = variantes.filter(v => v.color.toLowerCase() === color)
...
function actualizarSEO(p = {}) {
  ...
  // Meta dinámico para OpenGraph (si existe en index.html)
