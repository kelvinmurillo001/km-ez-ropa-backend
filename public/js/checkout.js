...
const API_PAYPAL_CREATE = `/api/paypal/create-order`;
...
form?.addEventListener("submit", async (e) => {
  ...
  try {
    ...
    for (const item of carrito) {
      const resProd = await fetch(`/api/products/${encodeURIComponent(item.id)}`);
      ...
      if (producto.variants?.length > 0) {
        const variante = producto.variants.find(v =>
          v.talla.toLowerCase() === talla && v.color.toLowerCase() === color && v.stock >= cantidad
        );
        if (!variante)
          throw new Error(`❌ Variante no disponible: ${sanitize(item.nombre)} - ${item.talla} - ${item.color}`);
      } else {
        const stock = producto.stockTotal || 0;
        if (stock < cantidad)
          throw new Error(`❌ Stock insuficiente: ${sanitize(item.nombre)}`);
      }
