...
async function cargarCategorias() {
  try {
    ...
    categoriaInput.innerHTML = `<option value="">Selecciona categoría</option>` +
      data.map(cat => `<option value="${sanitize(cat.name)}">${sanitize(cat.name)}</option>`).join("");

    categoriaInput.addEventListener("change", () => {
      const seleccionada = categoriasDisponibles.find(c => c.name === categoriaInput.value);
      subcategoriaInput.innerHTML = seleccionada?.subcategories?.length
        ? `<option value="">Selecciona subcategoría</option>` +
          seleccionada.subcategories.map(sub => `<option value="${sanitize(sub)}">${sanitize(sub)}</option>`).join("")
        : `<option value="">Sin subcategorías</option>`;
      subcategoriaInput.disabled = !(seleccionada?.subcategories?.length);
    });
  } catch (err) {
...
imagenInput.addEventListener("change", () => {
  ...
  previewPrincipal.innerHTML = `<img src="${url}" alt="Vista previa" class="imagen-preview-principal" />`;
});
...
form.addEventListener("submit", async (e) => {
  ...
  try {
    ...
    if (!categoria || !tallaTipo) throw new Error("⚠️ Categoría y tipo de talla obligatorios");
