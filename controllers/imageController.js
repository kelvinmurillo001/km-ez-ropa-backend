...
/**
 * 📂 Listar imágenes de Cloudinary desde una carpeta configurada
...
export const deleteImage = async (req, res) => {
  try {
    ...
    // Validación estricta de formato para evitar inyecciones
    const validPublicId = /^[a-zA-Z0-9_\-/]+$/
    if (!validPublicId.test(publicId)) {
      return res.status(400).json({
        ok: false,
        message: '⚠️ publicId inválido. Solo se permiten letras, números, guiones y slash.'
      })
    }
